...
import {
  ...
  KeyboardAvoidingView, Platform, ImageBackground, ScrollView, ActivityIndicator
} from 'react-native';
...
export default function LoginScreen() {
  ...
  const { autenticar, usuarioAtual, loading } = useUsuarios();
  ...
  // ==========================================
  const VALOR_SECRETO = '777'; // Valor para desbloquear o login
  // ==========================================
  ...
  const handleEntrar = async () => {
    ...
    if (res.sucesso) {
      Alert.alert('Sucesso', `Bem-vindo, ${res.usuario.nome}!`);
      router.replace('/(tabs)');
    } else {
      Alert.alert('Acesso Negado', res.error || 'Erro ao autenticar');
      setSenha('');
    }
  };
              ...
              >
                ...
                {desbloqueado && (
                  <View style={styles.secretBox}>
                    ...
                    <TouchableOpacity
                      style={[styles.btnAcessar, loading && styles.btnAcessarDisabled]}
                    ...
                    <Text style={styles.infoText}>
                      💡 Dica: Contate o Diretor para obter suas credenciais de acesso
                    </Text>
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  btnAcessarDisabled: { opacity: 0.6 },
